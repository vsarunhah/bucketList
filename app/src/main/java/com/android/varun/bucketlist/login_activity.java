...
public class login_activity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener{
        ...
        mAuthListener = new FirebaseAuth.AuthStateListener() {
            ...
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) {
                ...
                if (user != null) {
                    ...
                    startActivity(i);
                    Log.d("login_activity", "onAuthStateChanged:signed_in:" + user.getUid());
                } else {
                    // User is signed out
    ...
    public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
        String cr = connectionResult.toString();
        Log.d("cool", cr);
    ...
    private void firebaseAuthWithGoogle(GoogleSignInAccount acct) {
        Log.d("login_activity", "firebaseAuthWithGoogle:" + acct.getId());
        ...
        mAuth.signInWithCredential(credential)
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        Log.d("login_activity", "signInWithCredential:onComplete:" + task.isSuccessful());

                        // If sign in fails, display a message to the user. If sign in succeeds
                        // the auth state listener will be notified and logic to handle the
                        // signed in user can be handled in the listener.
                        if (!task.isSuccessful()) {
                            Log.w("login_activity", "signInWithCredential", task.getException());
